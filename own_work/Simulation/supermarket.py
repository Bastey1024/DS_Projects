...
class Customer:
    ...
    def move_dairy(self):
        ...
        if self.x>235 - (random.randint(0, 150)):
         ...
         if self.x < 205:
                self.y+=-1
         current_location='dairy'

    def move_upper_hall(self):
        if self.y>60- (random.randint(0, 150)):
            self.y+=-1
            if self.x<600:
                self.x+=+1
    # def move
...
while True:
    ...
    cv2.putText(frame,str('2019-09-04 01-32-07')  , (20,30), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,255,0),2,cv2.LINE_AA)
    ...
    peter.draw(frame)
    peter.move_dairy()
    # if peter(current_location='dairy'):
